...
class FooterSpec extends UnitSpec with OneAppPerSuite with FrontendTemplateSetup {

  "Footer" should {
    "contain links to assets-frontend JS" in new FrontendTemplateSetup {
    ...
    "contain links to a specified version of assets-frontend JS" in new FrontendTemplateSetup {
    ...
    "allow additional footer links to be specified SDT 477" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "allow multiple additional footer links to be specified SDT 477" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "show modified terms and condition link if 'termsAndConditionFooterLink' has value" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "show default terms and condition link if 'termsAndConditionFooterLink' has no value" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "support additional script elements in the footer of the page SDT 578" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "support multiple additional script elements in the footer of the page SDT 578" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "scriptElems" -> Seq(
    ...
    "support SSO Url SDT-473" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "no ssoUrl script when no ssoUrl given SDT-473" in new FrontendTemplateSetup {
      outputText should not include("""<script type="text/javascript">var ssoUrl = """)
    }

    "not show Google Analytics snippet when no googleAnalytics given SDT-475" in new FrontendTemplateSetup {
      outputText should not include("""(window,document,'script','//www.google-analytics.com/analytics.js','ga')""")
    }

    "show Google Analytics snippet when googleAnalytics trackingId given SDT-475" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "do not add ga set if no gaSetParams given" in new FrontendTemplateSetup {
      outputText should not include("""ga('set'""")
    }

    "set the correct dimensions in ga" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "googleAnalytics" -> Map(
          "trackingId" -> "random",
          "authProvider" -> "IDA",
    ...
    "not set the dimensions in ga when the data is blank" in new FrontendTemplateSetup {
    ...
    "support custom cookie domain for Google Analytics snippet SDT-475" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "googleAnalytics" -> Map(
    ...
    "support inline script elements in the footer of the page" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
