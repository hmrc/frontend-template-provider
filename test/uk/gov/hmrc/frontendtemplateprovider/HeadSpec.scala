...
class HeadSpec extends UnitSpec with OneAppPerSuite with FrontendTemplateSetup {

  "Head" should {
    "contain IE links" in new FrontendTemplateSetup {
    ...
    "contain links to assets-frontend CSS" in new FrontendTemplateSetup {
    ...
    "contain links to a specified version of assets-frontend CSS" in new FrontendTemplateSetup {
    ...
    "contain a body opening tag that does not contain a class if `bodyClass` is not specified" in new FrontendTemplateSetup {
      val body = bodyTagRegex.findFirstIn(outputText).get
      body should not include("class")
    }

    "contain a body opening tag that contains a `bodyClass` if one is specified " in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "bodyClass" -> "clazz"
      )
    ...
    "contains no optimizely script SDT-471" in new FrontendTemplateSetup {
      outputText should not include("optimizely")
      outputText should not include("<script src=''") // should not include a script with no src
    }

    "contain no optimizely script if only baseUrl is provided SDT-471" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "contain optimizely script if only projectId is provided SDT-471" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "contain optimizely script if both url and projectId is provided SDT-470" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "optimizelyBaseUrl" -> "cdn.optimizely.com/",
        "optimizelyProjectId" -> "id123"
      )
    ...
    "contain default title if not specified SDT 480" in new FrontendTemplateSetup {
      outputText should include("GOV.UK - The best place to find government services and information")
    }

    "contain default title if not specified in SDT 480" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "contain no headScripts even if you specify them SDT 472" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "contain link elem if url is passed through in a list of maps SDT-552" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "contain link elem for ie if IE is passed as a condition" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "linkElems" -> Map(
    ...
    "support specifying print media type for linkElems SDT-552" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
    ...
    "contain multiple link elems multiple elements in the list SDT-552" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "linkElems" -> List(
    ...
    "not contain navigation element if there is no navTitle specified SDT-474" in new FrontendTemplateSetup {
    ...
    "contain correct navigation header when only navTitle is specified SDT-474" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "navTitle" -> "My service"
      )
    ...
    "service-info included by default" in new FrontendTemplateSetup {
      outputText should include("""<div class="service-info">""")
    }

    "Beta banner feedback form should be authenticated if user is authenticated" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "betaBanner" -> true,
    ...
    "Beta banner feedback form should be authenticated if user is not authenticated" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "betaBanner" -> true,
        "feedbackIdentifier" -> "test-service",
    ...
    "Beta banner feedback form should be authenticated if user is not specified" in new FrontendTemplateSetup {
      override lazy val inputMap = Map(
        "betaBanner" -> true,
        "feedbackIdentifier" -> "test-service"
      )
